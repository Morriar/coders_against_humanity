...
//
// usage:
//	node loaddb.js db_name collection_name /path/to/json
//
// Pass "drop" as filename to drop the database:
//	node loaddb.js db_name collection_name drop

var fs = require("fs");
var mongo = require('mongoskin');

// Insert items into db.collection.
function loadCards(db, collection, json) {
	json.blackCards.forEach(function(card) {
	...
	json.whiteCards.forEach(function(word) {
		var obj = {
			color: "white",
			word: word
		}
...
if(argv.length != 5) {
	console.log("usage:\n");
	console.log("node loadCards.js db_name collection_name path/to/json");
	process.exit(1);
}
...
if(file === "drop") {
	...
	db.collection(collection_name).count(function(err, count) {
		console.log(count + " " + collection_name +"...")
		process.exit(0);
	});
} else {
	loadCards(db, collection_name, readJsonFile(file));
}
